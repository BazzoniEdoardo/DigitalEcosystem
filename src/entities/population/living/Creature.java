package entities.population.living;
...
import entities.population.living.logic.Brain;
...
public class Creature extends AbstractRenderedEntity implements SimulationEntity, Updatable, Serializable {
    @Serial
    ...
    public Creature(final Position position, final float energy, final float hunger, final DNA dna, final WorldContext world) {
        this.id = EntityManager.nextId();
    ...
    public void eat(final Food food) {
        ...
        this.hunger = Math.clamp(hunger + food.getNutrition() * dna.getMetabolismGene().getGeneAttribute("digestionMultiplier"), 0, dna.getMetabolismGene().getGeneAttribute("maxHunger"));
    ...
    //IDEA: fare un ciclo while dove il brain continua a chiamare la funzione decide, passandogli in input una ActionScore, fino al caso base,
    //cosi' si sviluppa una sorta di decision tree
    protected void updateAction() {
        ...
        switch (action) {
            ...
            case MOVE -> brain.movementDecision();
        }
    }
